...
impl UI {
    ...
    fn draw_input_dialog(&self, frame: &mut Frame, input_state: &crate::menu::InputDialogState) {
        ...
        frame.render_widget(Block::default().style(background_style).borders(Borders::ALL), dialog_area);
        
        // Split into sections: title, prompt, input, spacing, buttons
        let inner = dialog_area.inner(Margin { horizontal: 1, vertical: 1 });
        let dialog_chunks = Layout::default()
            .direction(Direction::Vertical)
            .constraints([
        ...
        if input_state.cursor_position == input_state.input.len() {
            input_spans.push(Span::styled(
                "_",
                Style::default().bg(input_bg).fg(Color::Yellow).add_modifier(Modifier::SLOW_BLINK),
            ));
        } else {
            ...
            if input_state.cursor_position < input_spans.len() {
                let ch = input_state.input.chars().nth(input_state.cursor_position).unwrap_or(' ');
                input_spans[input_state.cursor_position] = Span::styled(ch.to_string(), cursor_style);
            }
        }
        ...
        let cancel_style = if input_state.hovered_button == Some(1) {
            Style::default().bg(Color::DarkGray).fg(Color::Red).add_modifier(Modifier::BOLD)
        } else {

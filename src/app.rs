...
use crate::menu::{FileItem, MenuState, MenuSystem};
use crate::tab::{Tab, TabManager};
use crate::tree_view::TreeView;
use crate::ui::{ScrollbarState, UI};
...
impl App {
    ...
    fn save_current_file(&mut self) {
        ...
        if let Some(tab) = self.tab_manager.active_tab_mut() {
            if let Some(path) = &tab.path.clone() {
                if std::fs::write(path, tab.buffer.to_string()).is_ok() {
                    tab.mark_saved();
    ...
    pub fn handle_key_event(&mut self, key: KeyEvent) {
                ...
                (KeyCode::Enter, KeyModifiers::NONE) => {
                    if let Some(action) = self.menu_system.handle_enter() {
                        match action.as_str() {
                            ...
                            "new_file" => {
                                if let MenuState::TreeContextMenu(ref context_state) =
                            ...
                            "rename" => {
                                    ...
                                    &self.menu_system.state
                                {
                                    ...
                                    if let MenuState::InputDialog(ref mut input_state) =
                            ...
                            "delete" => {
                                    ...
                                    &self.menu_system.state
                                {
                                    ...
                                    let filename = target_path
                                        ...
                                        .unwrap_or("item")
                                        .to_string();
                                    let file_type = if context_state.is_directory {
                                        "directory"
                                    } else {
                                        "file"
                                    };
                            ...
                            "copy" => {
                                if let Some(tree_view) = &mut self.tree_view {
                                        ...
                                        &self.menu_system.state
                                    {
                                        ...
                                        if let Some(index) =
                                            tree_view.find_item_index(&context_state.target_path)
                                        {
                    ...
                    | (KeyCode::Char('r'), KeyModifiers::CONTROL) => {
                        ...
                        if let Some(ref mut tree_view) = self.tree_view {
                            let current_selected =
                                tree_view.get_selected_item().map(|item| item.path.clone());
                            tree_view.refresh();
                    ...
                    (KeyCode::Delete, KeyModifiers::NONE) if !tree_view.is_searching => {
                        ...
                        if let Some(selected_item) = tree_view.get_selected_item() {
                            let filename = selected_item
                                ...
                                .unwrap_or("item")
                                .to_string();
                            let file_type = if selected_item.is_dir {
                                "directory"
                            } else {
                                "file"
                            };
        ...
        let needs_viewport_update = if let Some(tab) = self.tab_manager.active_tab_mut() {
            ...
            let old_cursor_pos = tab.cursor.position;

            // Check if this is a modification command that needs state saving
            let should_save_state = matches!(
                key.code,
    ...
    pub fn handle_mouse_event(&mut self, mouse: MouseEvent) {
        ...
        if matches!(
            ...
            if matches!(
                self.menu_system.state,
                crate::menu::MenuState::FilePicker(_)
            ) {
            ...
            MouseEventKind::Down(MouseButton::Left) => {
                    ...
                    (self.last_click_time, self.last_click_pos)
                {
                    now.duration_since(last_time) < Duration::from_millis(500)
                        && (current_pos.0.abs_diff(last_pos.0) <= 1
                            && current_pos.1.abs_diff(last_pos.1) <= 1)
                } else {
                    false
                };
                ...
                if let Some(pos) =
                    self.mouse_to_text_position(mouse.column, mouse.row, active_index)
                {
                    if let Some(tab) = self.tab_manager.active_tab_mut() {
    ...
    fn handle_mouse_on_dialog(&mut self, mouse: MouseEvent) {
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                if let Some(button_clicked) =
                    self.get_dialog_button_at_position(mouse.column, mouse.row)
                {
    ...
    fn get_dialog_button_at_position(&self, mouse_x: u16, mouse_y: u16) -> Option<usize> {
        if let Some(message) = &self.warning_message {
            ...
            if mouse_x < popup_x
                || mouse_x >= popup_x + popup_width
                || mouse_y < popup_y
                || mouse_y >= popup_y + popup_height
            {
                return None;
            }
            ...
            MouseEventKind::Moved => {
                ...
                match &self.menu_system.state {
                    crate::menu::MenuState::MainMenu(menu) => {
                        ...
                        let hovered_item =
                            menu.get_clicked_item(&menu_area, mouse.column, mouse.row);
                    ...
                    crate::menu::MenuState::CurrentTabMenu(menu) => {
                        ...
                        if let crate::menu::MenuState::CurrentTabMenu(menu) =
                            &mut self.menu_system.state
                        {
                            menu.hovered_index = hovered_item;
                        }
                    ...
                    crate::menu::MenuState::TreeContextMenu(context_state) => {
                        ...
                        let hovered_item = context_state.menu.get_clicked_item(
                            &menu_area,
                            mouse.column,
                            mouse.row,
                        );

                        if let crate::menu::MenuState::TreeContextMenu(context_state) =
                            &mut self.menu_system.state
                        {
                ...
                match &self.menu_system.state {
                    crate::menu::MenuState::MainMenu(menu) => {
                            ...
                            menu.get_clicked_item(&menu_area, mouse.column, mouse.row)
                        {
                            ...
                            if let crate::menu::MenuState::MainMenu(menu) =
                                &mut self.menu_system.state
                            {
                    ...
                    crate::menu::MenuState::TreeContextMenu(context_state) => {
                        ...
                        if let Some(item_index) =
                            context_state
                                .menu
                                .get_clicked_item(&menu_area, mouse.column, mouse.row)
                        {
                                ...
                                if let Some(action) = self.menu_system.handle_enter() {
                                    match action.as_str() {
                                        ...
                                        "rename" => {
                                            ...
                                            // Pre-fill with current filename and select all
                                            if let crate::menu::MenuState::InputDialog(
                                                ref mut input_state,
                                            ) = &mut self.menu_system.state
                                            {
                                                input_state.input = filename.clone();
                                        ...
                                        "delete" => {
                                            ...
                                            let message =
                                                format!("Delete {} '{}'?", file_type, filename);
                                            self.warning_message = Some(message);
                                            self.warning_selected_button = 0; // Default to "No"
                                            self.warning_is_info = false; // Yes/No dialog
                                            self.pending_delete_path = Some(target_path);
    ...
    fn handle_mouse_on_file_picker(&mut self, mouse: MouseEvent) -> bool {
        ...
        match mouse.kind {
            ...
            MouseEventKind::Moved => {
                    ...
                    &mut self.menu_system.state
                {
                    ...
                    if mouse.column >= modal_x
                        && mouse.column < modal_x + modal_width
                        && mouse.row >= modal_y
            ...
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                let click_info = if let crate::menu::MenuState::FilePicker(picker_state) =
                    &self.menu_system.state
                {
                    // Calculate file picker modal area (same as in UI draw method)
                    let modal_width = 80u16.min(self.terminal_size.0.saturating_sub(4));
                        ...
                        && mouse.row < modal_y + modal_height
                    {
                           ...
                           mouse.row >= list_start_y && mouse.row < list_start_y + list_height
                        {
                            ...
                            let new_position =
                                scrollbar_state.click_position(list_height as usize, click_y);
                            let new_selected_index =
                                new_position.min(total_items.saturating_sub(1));
                            ...
                            Some((
                                ...
                                false,
                                new_selected_index,
                            )) // false means scrollbar click, not file selection
                        } else if mouse.row >= list_start_y
                            && mouse.row < list_start_y + list_height
                        {
                            ...
                            if clicked_file_index < picker_state.filtered_items.len() {
                                ...
                                let selected_item =
                                    picker_state.filtered_items[clicked_file_index].clone();
                                ...
                                Some((
                                    ...
                                    false,
                                    0,
                                )) // false means invalid click but inside modal
                            }
                        } else {
                            Some((
                                FileItem {
                                ...
                                false,
                                0,
                            )) // Click was inside modal but not on a file
                        }
                    } else {
    ...
    fn handle_scrollbar_click(&mut self, mouse: MouseEvent) {
        if let Some(tab) = self.tab_manager.active_tab_mut() {
            ...
            let scrollbar_state =
                ScrollbarState::new(content_lines, editor_height, tab.viewport_offset.0);
            ...
            match (key.code, key.modifiers) {
                ...
                (KeyCode::Left, KeyModifiers::CONTROL) | (KeyCode::Left, KeyModifiers::ALT) => {
                    if input_state.cursor_position > 0 {
                        ...
                        while pos > 0
                            && (chars[pos].is_whitespace() || is_word_separator(chars[pos]))
                        {
                            pos -= 1;
                        }
                ...
                (KeyCode::Char('c'), KeyModifiers::CONTROL) => {
                    if let Some(sel_start) = input_state.selection_start {
                        ...
                        let selected_text: String = input_state
                            .input
                            .chars()
                            .skip(start)
                            .take(end - start)
        ...
        match operation {
            "save_file" => {
                ...
                if let Some(tab) = self.tab_manager.active_tab_mut() {
                    ...
                    if std::fs::write(&file_path, tab.buffer.to_string()).is_ok() {
    ...
    fn delete_selection(
        buffer: &mut crate::rope_buffer::RopeBuffer,
    ...
    fn expand_tree_to_current_file(&mut self) {
        if let (Some(tree_view), Some(current_tab)) =
            (&mut self.tree_view, self.tab_manager.active_tab())
        {

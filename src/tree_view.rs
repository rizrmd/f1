...
impl TreeView {
    ...
    fn toggle_node_recursive(node: &mut TreeNode, target_path: &Path) -> Result<(), std::io::Error> {
    ...
    fn search_in_directory(&self, node: &TreeNode, query: &str, results: &mut Vec<(usize, TreeNode)>, index: &mut usize, max_depth: usize) {
    ...
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        if self.is_searching {
            ...
            for (i, ch) in search_text.chars().enumerate() {
                ...
                if x < inner.x + content_width {
                    let style = if i < 8 { // "Search: " part
                        Style::default().fg(Color::Yellow).bg(Color::DarkGray)
                    } else if i == search_text.len() - 1 { // Cursor
        ...
        for (display_index, item_index) in (start_index..end_index).enumerate() {
            if let Some(item) = visible_items.get(item_index) {
                ...
                let name_style = if is_selected {
                    ...
                    if item.is_dir {
                        Style::default().fg(Color::Rgb(80, 80, 80)) // Dimmed cyan
                    } else {
                        Style::default().fg(Color::Rgb(80, 80, 80)) // Dimmed white
                    }
                } else if item.is_dir {
                ...
                let display_name = if item.name.len() as u16 > max_name_width {
                    format!("{}...", &item.name[..max_name_width.saturating_sub(3) as usize])
                } else {
                    item.name.clone()
                };
                
                for (_i, ch) in display_name.chars().enumerate() {
                    if x < content_area.x + content_width {
                        buf[(x, y)]
                            .set_symbol(&ch.to_string())

...
use crate::ui::{VerticalScrollbar, ScrollbarState};
...
impl<'a> MarkdownWidget<'a> {
    pub fn parse_markdown(&self) -> Vec<Line<'static>> {
        ...
        while i < content_lines.len() {
            ...
            if line.trim().contains('|') && !line.trim().starts_with("```") && !line.trim().is_empty() {
    ...
    fn parse_line(&self, line: &str) -> Line<'static> {
        ...
        if line.trim().starts_with("- ") || line.trim().starts_with("* ") {
            ...
            let text_start = line.find(|c: char| c != ' ' && c != '-' && c != '*').unwrap_or(line.len());
            let list_text = if text_start < line.len() { &line[text_start..] } else { "" };
    ...
    fn create_table_border(&self, col_widths: &[usize], left: &str, mid: &str, right: &str, fill: &str) -> Line<'static> {
    ...
    fn create_table_row(&self, row: &[String], col_widths: &[usize], is_header: bool) -> Line<'static> {
        let mut spans = Vec::new();
